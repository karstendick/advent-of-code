...
for y, line in enumerate(lines):
  for x, c in enumerate(line):
    cells[(x, y, 0, 0)] = 1 if c == '#' else 0

print(cells)

def get_range(cells):
  xyzws = list(cells.keys())
  xs = [x for x,y,z,w in xyzws]
  ys = [y for x,y,z,w in xyzws]
  zs = [z for x,y,z,w in xyzws]
  ws = [w for x,y,z,w in xyzws]

  return min(xs), max(xs), min(ys), max(ys), min(zs), max(zs), min(ws), max(ws)


# def cells_string(cells):
#   xmin, xmax, ymin, ymax, zmin, zmax, wmin, wmax = get_range(cells)
...
def count_num_neighbors(cells, x, y, z, w):
  n = 0
  for dw in range(-1, 2):
    for dz in range(-1, 2):
      for dy in range(-1, 2):
    ...
    for z in range(zmin-1, zmax+2):
      for y in range(ymin-1, ymax+2):
        for x in range(xmin-1, xmax+2):
          old_cell = cells[(x,y,z,w)]
          num_neighbors = count_num_neighbors(cells, x, y, z, w)
          if old_cell == 1:
